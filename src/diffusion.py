...
class DDIMSpatialLatent:
    ...
    def sample(self, z_t, class_labels, t_steps=50, eta=0.0, verbose=False):
        timesteps = torch.linspace(self.num_steps - 1, 0, t_steps, dtype=torch.long, device=self.device)
        iterator = tqdm(timesteps, desc="DDIM") if verbose else timesteps  
        ...
        with torch.no_grad():
            for idx, step in enumerate(iterator):
                ...
                z_hat_0 = (1.0/sqrt_alpha_bar_t) * (z_t - sqrt_1_minus_alpha_bar_t * epsilon_pred) 
                ...
                z_t = sqrt_alpha_bar_t_prev * z_hat_0 + noise_direction * epsilon_pred
